...
##################################################################
#
# Processing Script for Suspension-Rates-by-Meal-Eligibility
...
backfill_years <- expand.grid(
  ...
  `Year` = years, 
  `Free/Reduced Price Meal Eligibility` = "Eligible for Free or Reduced-Price Meals"
)
...
complete_susp_rates_long <- complete_susp_rates_long %>% 
  select(District, FIPS, Year, `Free/Reduced Price Meal Eligibility`, `Measure Type`, Variable, Value) %>% 
  arrange(District, Year)

# Write to File
write.table(
  complete_susp_rates_long,
  file.path(path_to_top_level, "data", "suspension_rates_by_meals_2017.csv"),
  sep = ",",
  row.names = F
)
